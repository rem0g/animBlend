async function createScene(canvas, existingEngine) {
    console.log("Loading Scene!");

    // Use the engine passed from the HTML file if available
    var engine = existingEngine;
    
    // Only create a new engine if one wasn't provided
    // This prevents the "getContextAttributes is not a function" error
    if (!engine) {
        console.log("No existing engine provided, creating a new one");
        var options = {
        ...
        engine.disableManifestCheck = true; // disable manifest checking
    } else {
        console.log("Using existing engine from 3DViewer.html");
    }
...
var loadAssetMesh = async function (scene, path = basePathMesh + "Nemu/", fileName = "Nemu.glb", bugger = false) {
    ...
    console.log("Converted path type:", typeof pathString, "value:", pathString);
    console.log("Converted fileName type:", typeof fileNameString, "value:", fileNameString);
    ...
    try {
        const asset = {
            fetched: await BABYLON.SceneLoader.ImportMeshAsync(null, pathString, fileNameString, scene),
            root: null,
            faceMesh: null,
            teethMesh: null,
            hips: null,
        ...
        console.error("FileName:", fileNameString);
        return null;
    }
};
